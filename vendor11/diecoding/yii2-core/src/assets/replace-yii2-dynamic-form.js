...
(function($) {
  ...
  var methods = {
    init: function(widgetOptions) {
      _fixBugPlugins(widgetOptions, false);
    ...
    addItem: function(widgetOptions, e, $elem) {
      ...
      _fixBugPlugins(widgetOptions, true);
    },
  ...
  var _restoreSpecialJs = function(widgetOptions) {
    ...
    if ($hasTimepicker.length > 0) {
      $hasTimepicker.each(function() {
        ...
        $(this)
          .parent()
          .find(".bootstrap-timepicker-widget")
          .remove();
        $(this).unbind();
  ...
  var _fixBugPlugins = function(widgetOptions, isAdd) {
    var widgetOptionsRoot = _getWidgetOptionsRoot(widgetOptions);
    ...
    if ($hasSelect2.length > 0) {
      if ($("#diecoding-fix-bug-plugins").length) {
        var _max = $("#diecoding-fix-bug-plugins").val();
      } else {
        var _max = $hasSelect2.length;
        $("body").append(
          '<input id="diecoding-fix-bug-plugins" type="hidden" value="' +
            _max +
            '" style="display: none;"></input>'
        );
      }

      $hasSelect2.each(function(index, value) {
        var idS2Container = "select2-" + $(this).attr("id") + "-container";
        var s2Container = $("#" + idS2Container);
        if (s2Container.length == 0) {
          s2Container = $(s2Container.selector);
        }

        if (typeof s2Container[0] !== "undefined") {
          title = s2Container[0].title;
          s2Container.html(s2Container[0].innerHTML + title);
        }
      });

      if (isAdd) {
        for (var i = 0; i < $hasSelect2.length; i++) {
          if (i >= _max) {
            var _hasSelect2 = $hasSelect2[i];
            var s2Old = $("#" + $hasSelect2[0].id);
            var s2Copy = $("#" + _hasSelect2.id);
            var idS2Container = "select2-" + _hasSelect2.id + "-container";
            var s2Container = $("#" + idS2Container);
            if (
              typeof s2Old[0] !== "undefined" &&
              typeof s2Copy[0] !== "undefined" &&
              typeof s2Container[0] !== "undefined"
            ) {
              if (s2Old[0].value == s2Copy[0].value) {
                s2Copy[0].value = "";
              }
              s2Container[0].title = "";
              s2Container.html("");
