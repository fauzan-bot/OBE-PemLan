...
function ModalRemote(modalId) {
	 ...
	 */
	this.doRemote = function(url, method, data, showModal = true) {
		var instance = this;
		$.ajax({
			url: url,
			method: method,
			...
			async: false,
			beforeSend: function() {
				showModal ? beforeRemoteRequest.call(instance) : null;
			},
			error: function(response) {
				showModal ? errorRemoteResponse.call(instance, response) : null;
			},
	...
	this.open = function(elm, bulkData) {
		var showModal = true;
		if ($(elm).hasAttr("data-modal-show")) {
			showModal = $(elm).attr("data-modal-show") != "0";
		}
