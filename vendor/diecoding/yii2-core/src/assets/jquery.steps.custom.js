...
(function($, undefined) {
	$.fn.extend({
		...
		_enableAria: function(enable) {
			return enable == null || enable
				? this.removeClass("disabled alert-secondary")._aria(
						"disabled",
						"false"
				  )
				: this.addClass("disabled alert-secondary")._aria(
						"disabled",
						"true"
				  );
		},
		...
		_selectAria: function(select) {
			return select == null || select
				? this.addClass("current bg-primary")._aria("selected", "true")
				: this.removeClass("current bg-primary")._aria(
						"selected",
						"false"
				  );
		},
	...
	function finishStep(wizard, state) {
		...
		if (wizard.triggerHandler("finishing", [state.currentIndex])) {
			currentStep
				.addClass("done alert-success")
				.removeClass("error alert-danger");
			wizard.triggerHandler("finished", [state.currentIndex]);
		} else {
			currentStep.addClass("error alert-danger");
		}
	}
	...
	function refreshPagination(wizard, options, state) {
		if (options.enablePagination) {
			var finish = wizard.find(".actions #btn-submit").parent(),
				next = wizard.find(".actions a[href$='#next']").parent();

			if (!options.forceMoveForward) {
				var previous = wizard.find(".actions a[href$='#previous']");
	...
	function refreshStepNavigation(wizard, options, state, oldIndex) {
		...
		if (oldIndex != null) {
			...
			oldStepAnchor
				.parent()
				.addClass("done alert-success")
				.removeClass("disabled alert-secondary")
				.removeClass("error alert-danger")
				._selectAria(false);

			currentOrNewStepAnchor
				.parent()
				.addClass("disabled alert-secondary")
				.removeClass("done alert-success")
				.removeClass("error alert-danger")
				._selectAria(false);

			if (wizard.find(".is-invalid").length > 0) {
			...
			stepTitles
				.eq(oldIndex)
				.removeClass("current bg-primary")
				.next(".body")
				.removeClass("current");
	...
	function renderPagination(wizard, options, state) {
		if (options.enablePagination) {
			...
			if (!options.forceMoveForward) {
				buttons += '<li><a href="#{0}" class="btn btn-outline-secondary waves-effect waves-light" role="menuitem"><i class="mdi mdi-pan-left"></i> {1}</a></li>'.format(
					"previous",
					options.labels.previous
				);
			}

			buttons += '<li><a href="#{0}" class="btn btn-outline-primary waves-effect waves-light" role="menuitem">{1} <i class="mdi mdi-pan-right"></i> </a></li>'.format(
				"next",
				options.labels.next
			);

			if (options.enableFinishButton) {
				buttons += '<li><a href="#submit" id="btn-submit" class="btn btn-primary waves-effect waves-light" role="menuitem"><i class="mdi mdi-content-save-move"></i> {1}</a></li>'.format(
					"finish",
			...
			if (options.enableCancelButton) {
				buttons += '<li><a href="#submit" id="btn-submit" class="btn btn-secondary waves-effect waves-light" role="menuitem"><i class="mdi mdi-pan-cancel"></i> {1}</a></li>'.format(
					"cancel",
	...
	function renderTitle(wizard, options, state, header, index) {
		...
		if (state.currentIndex > index) {
			if (wizard.find(".is-invalid").length > 0) {
				stepItem.addClass("error alert-danger");
			} else {
				stepItem.addClass("done alert-success");
			}
		}
